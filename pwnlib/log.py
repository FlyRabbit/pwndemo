...
class Logger(object):
    ...
    * :meth:`warning_once` (alias :meth:`warn_once`)
    ...
    def indented2(self, message, *args, **kwargs):
        """indented2(message, *args, level = logging.INFO, **kwargs)

        Log a message but put more space than indented

        Arguments:
            level(int): Alternate log level at which to set the indenter
                        message.  Defaults to :const:`logging.INFO`.
        """
        level = kwargs.pop('level', logging.INFO)
        self._log(level, message, args, kwargs, 'indented2')
    ...
    def send(self, message, *args, **kwargs):
        ...
        Logs a send message.
        """
        self._log(logging.INFO, message, args, kwargs, 'send')

    def recv(self, message, *args, **kwargs):
        """recv(message, *args, **kwargs)

        Logs a recv message.
        """
        self._log(logging.INFO, message, args, kwargs, 'recv')
...
class Formatter(logging.Formatter):
    ...
    * If the message spans multiple lines they are split, and all subsequent
    ...
    def format(self, record):
        ...
        if msgtype == 'send':
            # just want add 4 space before send
            style, symb = _msgtype_prefixes[msgtype]
            prefix = '\n  [%s] ' % style(symb)
        elif msgtype in _msgtype_prefixes:
            style, symb = _msgtype_prefixes[msgtype]
            prefix = '\n[%s] ' % style(symb)
        elif msgtype == 'indented':
            prefix = self.indent
        elif msgtype == 'indented2':
            prefix = self.indent2
        ...
        msg = prefix + msg
        if msgtype == 'indented2':
            msg = self.nlindent2.join(msg.splitlines())
        elif msgtype == 'indented':
