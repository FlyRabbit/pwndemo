...
class tube(Timeout):
    ...
    def _fillbuffer(self, timeout = default):
        ...
        if data and dumplog.isEnabledFor(logging.INFO) and not self.close_info:
            ...
            if len(set(data)) == 1:
                dumplog.indented('%r * %#x' % (data[0], len(data)), level = logging.INFO)
            elif all(c in string.printable for c in data):
                for line in data.splitlines(True):
                    dumplog.indented(repr(line), level = logging.INFO)
            else:
                dumplog.indented(fiddling.hexdump(data), level = logging.INFO)
    ...
    def send(self, data):
        ...
        if dumplog.isEnabledFor(logging.INFO) and not self.close_info:
            ...
            if len(set(data)) == 1:
                ...
                for line in data.splitlines(True):
                    dumplog.indented2(repr(line), level = logging.INFO)
            else:
                dumplog.indented2(fiddling.hexdump(data), level = logging.INFO)
        self.send_raw(data)
